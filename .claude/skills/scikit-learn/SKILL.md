...
description: >
  General-purpose machine learning with scikit-learn. Use for classification,
  regression, unsupervised clustering, dimensionality reduction, model evaluation,
  hyperparameter tuning, and ML pipelines. Default skill for any ML task that
  does not require deep learning or omics-specific methods.
  Trigger phrases: "machine learning", "classify samples", "random forest",
  "SVM", "feature importance", "cross-validation", "train a model",
  "predict outcome", "clustering k-means", "PCA dimensionality reduction",
  "grid search", "ML pipeline", "hyperparameter tuning", "scikit-learn".
  Do NOT use for: survival analysis (use survival-analysis-clinical),
  biomarker LASSO panels (use lasso-biomarker-panel), Bayesian models (use pymc),
  scRNA-seq (use scrnaseq-scanpy-core-analysis).
category: machine_learning
short-description: "Classical ML with scikit-learn — classification, regression, clustering, CV, pipelines."
detailed-description: "Full scikit-learn workflow: preprocessing (scaling, encoding, imputation), supervised learning (RF, SVM, logistic regression, gradient boosting), unsupervised learning (k-means, hierarchical, PCA, UMAP), cross-validation (nested CV, stratified K-fold), hyperparameter tuning (GridSearchCV, RandomizedSearchCV, Optuna), and Pipeline API for reproducible ML workflows."
---
...
After model fitting, invoke these skills as needed:
- **`shap`** — explain any fitted model's predictions (feature importance, beeswarm, waterfall, force plots). Always offer SHAP after fitting a Random Forest, gradient boosting, or any black-box model.
- **`statistical-analysis`** — if the ML task is actually a hypothesis test in disguise (2-group comparison, ANOVA), use that skill instead.
