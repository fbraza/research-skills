---
id: scikit-survival
name: scikit-survival ML Survival Analysis
description: >
  ML-based survival analysis with scikit-survival. Use for Random Survival Forests,
  Gradient Boosting Survival Analysis, penalized Cox (elastic net), and comprehensive
  evaluation (C-index, time-AUC, Integrated Brier Score). Complements the R-based
  survival-analysis-clinical skill — use this when ML survival models or high-dimensional
  omics-to-survival tasks are needed.
  Trigger phrases: "Random Survival Forest", "RSF survival", "gradient boosting survival",
  "scikit-survival", "ML survival model", "C-index", "Brier score survival",
  "time-dependent AUC survival", "penalized Cox Python", "CoxnetSurvivalAnalysis",
  "high-dimensional survival", "omics survival prediction".
  Do NOT use for: standard KM/Cox clinical survival (use survival-analysis-clinical),
  LASSO biomarker panels (use lasso-biomarker-panel).
category: clinical_translational
short-description: "ML survival models in Python — RSF, GBS, penalized Cox, C-index, Brier score."
detailed-description: "Complete scikit-survival workflow: data preparation (Surv structured arrays), Random Survival Forest with permutation-based feature importance, Gradient Boosting Survival Analysis, CoxnetSurvivalAnalysis (elastic net), evaluation (Harrell/Uno C-index, time-dependent AUC, Integrated Brier Score), nested cross-validation, model comparison. Use scripts/ml_survival.py as the starting point."
license: GPL-3.0 license
metadata:
    skill-author: K-Dense Inc. (scripts added by Aria)
---
...
| Script | Purpose |
|---|---|
| `scripts/ml_survival.py` | Full workflow: data prep (Surv arrays), RSF, GBS, penalized Cox, evaluation, feature importance, CV, model comparison |

**Run:**
```bash
python scripts/ml_survival.py
```
Outputs saved to `./results/ml_survival/`.

## Reference Documentation

| File | Content |
|---|---|
| `references/ensemble-models.md` | RSF, GBS, ExtraSurvivalTrees — parameters, tuning, early stopping |
| `references/cox-models.md` | CoxPHSurvivalAnalysis, CoxnetSurvivalAnalysis, IPCRidge (AFT) |
| `references/evaluation-metrics.md` | C-index (Harrell vs Uno), time-AUC, Brier score — when to use each |
| `references/competing-risks.md` | Competing risks in Python |
| `references/data-handling.md` | Structured array format, missing data, censoring |
| `references/svm-models.md` | Survival SVM |
