...
description: >
  Use this skill when the user wants to compare two groups of ChIP-seq, ATAC-seq, DNase-seq, or
  Bisulfite-seq experiments to find differential peaks or differentially methylated regions using
  ChIP-Atlas. Trigger phrases: "differential peaks between conditions", "ChIP-Atlas differential
  analysis", "differential chromatin accessibility", "differentially methylated regions", "DMR",
  "DPR", "compare ChIP-seq experiments edgeR", "differential peak regions between two groups".
  Requires ChIP-Atlas accession IDs for two groups. Do NOT use for enrichment of a gene list
  near ChIP peaks (use chip-atlas-peak-enrichment) or for finding target genes of one TF
