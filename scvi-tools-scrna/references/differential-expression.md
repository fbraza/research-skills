...
## Bayesian DE vs Pseudobulk DE

### When to Use Which Approach

| Aspect | scvi-tools Bayesian DE | Pseudobulk DE (DESeq2) |
|--------|----------------------|----------------------|
| **Unit of analysis** | Individual cells | Aggregated per sample |
| **Statistical model** | Bayesian posterior sampling | Negative binomial GLM |
| **Best for** | Marker gene discovery, cell type characterization | Condition comparisons with biological replicates |
| **Requires** | Trained scVI/scANVI model | ≥2 biological replicates per condition |
| **Handles batch effects** | Via model (integrated during training) | Via design formula |
| **Pseudoreplication risk** | Yes — treats cells as independent | No — aggregates to sample level |
| **Gold standard for** | Exploratory analysis, one-vs-rest markers | Inferential condition comparisons |

**Recommendation:** Use scvi-tools Bayesian DE for discovering marker genes and characterizing cell types (one-vs-rest or pairwise between cell types). Use pseudobulk DE (DESeq2) for formal condition comparisons (disease vs. healthy) when biological replicates are available. For pseudobulk DE, see the scanpy skill's `pseudobulk_de_guide.md`.

**Critical caveat:** Cell-level DE (including scvi-tools Bayesian DE) inflates false positive rates for condition comparisons because cells from the same sample are not independent observations. This is well documented (Squair et al. 2021, Nature Communications).
...
11. **Know the limits**: Use pseudobulk DE (DESeq2) for formal condition comparisons with biological replicates
