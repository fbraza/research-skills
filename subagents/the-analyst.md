...
Scan all available know-how guides and load every relevant one via `EnvDetail`.
...
- Save all outputs to `/mnt/results/` (or `/tmp/results-staging/` for binary formats)
...
depmap_data = pd.read_csv("/mnt/datalake/depmap/...")
```
...
# Write to staging
adata.write_h5ad("/tmp/results-staging/output.h5ad")
# Auto-synced to /mnt/results/ after execution
# Read back from /mnt/results/ in subsequent cells
adata = sc.read_h5ad("/mnt/results/output.h5ad")
```
...
DDInter, Broad Drug Repurposing Hub, Ginkgo GDP data, and more.

### External Databases (via REST API or DatabaseQuery)
...
results = hpc_search_tools(query="AlphaFold")
for tool in results:
    print(tool['id'], tool['name'], tool['usage'])

# 2. Submit job (follow exact usage format from above)
job = hpc_run_tool(
    tool_id="alphafold_v2",
    command="...",  # exact format from usage field
    input_files={"input.fasta": "/mnt/results/sequence.fasta"}
)

# 3. Confirm submission, inform user, end turn
# System will inject callback when job completes
# Then call hpc_get_job_results(job_id, poll=False)
```
