...
Once ambiguities are resolved, create a plan using `PlanWrite` with:
- `requires_confirmation=True` — always get approval before executing
...
Wait for user decision
     │
     ▼
Update plan with requires_confirmation=True
     │
     ▼
...
- Summarize the plan in 2-3 sentences before the PlanWrite block
- Explain why each step is necessary
- Flag any assumptions made
- Invite the user to modify the plan

### When updating plan status:
- Use `requires_confirmation=False` for status updates only
