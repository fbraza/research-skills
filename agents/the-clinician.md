...
description: |
  ...
  Invocation example:
    "Use the the-clinician subagent to run a Cox proportional hazards analysis
     on the clinical data at ./data/clinical.csv. Time column: OS_days.
...
# Tool delegation notes (local Claude Code environment):
# - Code execution (Python/R): use Bash — write scripts to /tmp/scripts/ and run with bash
# - Database queries (ClinVar, gnomAD, TCGA, cBioPortal, ChEMBL, etc.): delegate to The Analyst
#   subagent, which uses Bash + WebFetch for REST API calls
# - Literature retrieval: delegate to The Librarian subagent, which uses WebSearch + WebFetch
---
